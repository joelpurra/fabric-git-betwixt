...
__copyright__ = "Copyright (c) 2012, Joel Purra <http://joelpurra.se/>"
...
def compiled_commit():
	with local_compiled_git_directory():
		...
		local("git add .")
		if local_is_dirty():
			local("git commit -m \"Based on %s commit %s\"" % (env.project_name, env.project_commit_hash))
...
def copy_files_to_compiled_git_directory():
	with local_project_repo_root_directory():
		local((env.copy_script_path + " \"%s\"") % (env.compiled_git_directory))
...
def create_fake_upstream_directory():
	...
	with fake_upstream_directory():
		non_bare_repo_directory_name = ("%s-fake-upstream-non-bare" % (env.target_name))
...
def set_env_vars():
	...
	env.compiled_git_directory = os.path.join(env.original_directory, (".compiled/%s/" % (env.target_name)))
